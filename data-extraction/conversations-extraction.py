# ------------------------------------------------------------------------------
# This code extracts IndexedDB records with object_store_name "conversations"
...
def conversations_extraction(path, output_json):
    """
    Extracts all IndexedDB records with the same database_id as the record
    with object_store_name == "conversations" and with index_id=1.
    ...
    # First step: find all database_id for "conversations" and build database_id -> tenantId mapping
    database_ids = set()
    dbid_to_tenantid = {}
    records = []
    def save_nameskey_records(record_iter):
        for rec in record_iter:
            key = rec.key
            # Map database_id to tenantId from DatabaseNameKey
            if getattr(key, "__class__", None) and key.__class__.__name__ == "DatabaseNameKey":
                db_name = getattr(key, "database_name", None)
                dbid = getattr(rec, "value", None)
                if db_name and dbid is not None:
                    parts = db_name.split(":")
                    if len(parts) > 3:
                        tenant_id = parts[3]
                        dbid_to_tenantid[dbid] = tenant_id
            # Find database_ids for conversations
            if hasattr(key, "object_store_name") and getattr(key, "object_store_name", None) == "conversations":
    ...
    if not database_ids:
        print("No database_id found for object_store_name='conversations'")
        with open(output_json, "w", encoding="utf-8") as f:
            json.dump(records, f, ensure_ascii=False, indent=2, sort_keys=True)
        print(f"Saved {len(records)} records to {output_json}")
        return

    # Second step: extract all desired records for each found database_id, annotate with tenantId
    def save_datakey_records(record_iter, path_str):
        for rec in record_iter:
            ...
            if kp and kp.database_id in database_ids and kp.index_id == 1:
                try:
                    tenant_id = dbid_to_tenantid.get(kp.database_id)
                    records.append({
                        "offset": rec.offset,
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Extract IndexedDB conversations records")
    parser.add_argument("input_path", help="IndexedDB/LevelDB file or folder")
    parser.add_argument("output_json", help="Output JSON file")
    args = parser.parse_args()
    conversations_extraction(args.input_path, args.output_json)
