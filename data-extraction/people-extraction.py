# ------------------------------------------------------------------------------
# This code extracts IndexedDB records with object_store_name "profiles"
...
def profiles_extraction(path, output_json):
    """
    Extracts all IndexedDB records with the same database_id as the record
    with object_store_name == "profiles" and with index_id=1.
    ...
    def save_nameskey_records(record_iter):
        for rec in record_iter:
            key = rec.key
            if hasattr(key, "object_store_name") and getattr(key, "object_store_name", None) == "profiles":
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Extract IndexedDB profiles records")
    parser.add_argument("input_path", help="IndexedDB/LevelDB file or folder")
    parser.add_argument("output_json", help="Output JSON file")
    args = parser.parse_args()
    profiles_extraction(args.input_path, args.output_json)
