...
### 7. Microsoft Teams Group Chats (`TSK_TEAMS_GROUP_CHAT`)
...
### 8. Microsoft Teams Private Chats (`TSK_TEAMS_PRIVATE_CHAT`)
...
### 9. Microsoft Teams Channels (`TSK_TEAMS_TEAMS_CHAT`)
...
| Attribute Name | TSK Type | Description |
| --- | --- | --- |
| Member ID | `TSK_TEAMS_MEMBER_ID` | Member's unique identifier with enriched display name in format "ID (DisplayName)". |
| Thread ID | `TSK_TEAMS_MEMBER_THREAD_ID` | Thread ID this member belongs to. |
| Member Role | `TSK_TEAMS_MEMBER_ROLE` | Member's role within the thread (e.g., owner, member). |
| Display Name | `TSK_TEAMS_MEMBER_DISPLAYNAME` | Member's display name. |
